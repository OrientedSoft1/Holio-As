...
export default function GitHub() {
  ...
  const [showRepoExistsDialog, setShowRepoExistsDialog] = useState(false);
  ...
  const handlePushEntireProject = async () => {
    const baseRepoName = 'Holio-As';
    
    try {
      // Check if repo already exists
      toast.info('Checking repository...');
      
      // First get the authenticated user to get the owner
      const userReposResponse = await apiClient.list_repositories({
        visibility: 'all',
        sort: 'updated',
        per_page: 1,
        page: 1
      ...
      if (userRepos.length === 0) {
        toast.error('Could not determine GitHub username. Please create a repository first.');
        return;
      }
      
      const owner = userRepos[0].full_name.split('/')[0];
      
      const checkResponse = await apiClient.check_repository_exists({ owner, repo: baseRepoName });
      const checkData = await checkResponse.json();
      
      if (checkData.exists) {
        // Repo exists - show choice dialog
        setExistingRepoUrl(checkData.repository.html_url);
        setShowRepoExistsDialog(true);
        return;
      }
      
      // Repo doesn't exist - create new one
      await deployToGitHub(baseRepoName, false);
      
    } catch (error: any) {
      console.error('Failed to check repository:', error);
      toast.error(error?.message || 'Failed to check repository');
    }
  };
  
  const deployToGitHub = async (repoName: string, isUpdate: boolean) => {
    try {
      setPushingProject(true);
      setShowRepoExistsDialog(false);
      
      let owner: string;
      let finalRepoName: string;
      
      if (isUpdate) {
        // Update existing repo
        finalRepoName = repoName;
        // Get owner from user repos
        const userReposResponse = await apiClient.list_repositories({
          visibility: 'all',
          sort: 'updated',
          per_page: 1,
        ...
        if (userRepos.length === 0) {
          toast.error('Could not determine GitHub username.');
          return;
        }
        
        owner = userRepos[0].full_name.split('/')[0];
        
        toast.info(`Updating existing repository "${finalRepoName}"...`);
      } else {
        // Create new repository
        finalRepoName = repoName;
        toast.info(`Creating repository "${finalRepoName}"...`);
        ...
        owner = newRepo.full_name.split('/')[0];
        toast.success('Repository created!');
      }
      
      // Use real project files from backend
      ...
      // Push all files in atomic commit
      ...
      const pushResponse = await apiClient.push_files({
        ...
        message: isUpdate 
          ? `Update Riff project (${new Date().toISOString()})`
          : 'Initial commit - Full Riff AI Studio project',
        branch: 'main'
      });
      ...
      toast.success(
        <div>
          <p className="font-semibold">
            {isUpdate ? 'Project updated on GitHub! 🎉' : 'Project deployed to GitHub! 🚀'}
          </p>
          <p className="text-sm text-muted-foreground">{pushData.repository_url}</p>
        </div>,
        { duration: 5000 }
      );
      ...
      window.open(pushData.repository_url, '_blank');
  ...
  return (
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <AlertDialog open={showRepoExistsDialog} onOpenChange={setShowRepoExistsDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Repository Already Exists</AlertDialogTitle>
              <AlertDialogDescription>
                The repository "Holio-As" already exists at:
                <br />
                <a 
                ...
                <br /><br />
                Would you like to update it with the latest code, or create a new repository with a timestamp?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                onClick={() => {
                  ...
                  deployToGitHub(`Holio-As-${timestamp}`, false);
                }}
                variant="outline"
              >
              ...
              <AlertDialogAction
                onClick={() => deployToGitHub('Holio-As', true)}
              >
                Update Existing
              </AlertDialogAction>
            </AlertDialogFooter>
